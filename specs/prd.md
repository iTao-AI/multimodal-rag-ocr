...
每个用户可触发的操作（上传、问答、删除、创建知识库）必须实现以下 6 个状态：

| 状态 | 何时出现 | 用户看到 |
|------|----------|----------|
| loading | 请求发出到响应到达前 | 进度指示器 + 操作按钮禁用 |
| success | 2xx 响应 | 成功提示 + 刷新数据 |
| error | 4xx/5xx 响应 | 错误消息 + 重试按钮 |
| empty | 2xx 响应但数据为空 | "暂无数据"提示 + 引导操作 |
| stale | 后台请求超时（>5s） | 显示旧数据 + "数据可能过期"标签 |
| partial | 部分成功（如多文件上传部分失败） | 成功列表 + 失败列表 |
...
### Chat Service 降级链（6 个外部依赖失败路径）

Chat Service 依赖 6 个外部组件，降级优先级从高到低：

```
用户提问
  │
  ├─ ① Redis 缓存命中? ──是──→ 直接返回缓存答案
  │         │否
  │         ▼
  ├─ ② Query Rewrite 可用? ──是──→ 生成 3 个查询变体
  │         │否                    （默认降级：使用原始查询）
  │         ▼
  ├─ ③ Milvus 可用? ──否──→ 跳过检索，直接调 LLM 回答（无引用）
  │         │是
  │         ▼
  ├─ ④ BM25 可用? ──是──→ 向量 + BM25 RRF 融合
  │         │否
  │         ▼                （默认降级：纯向量排序）
  ├─ ⑤ Reranker 可用? ──是──→ 精排 Top-5
  │         │否
  │         ▼                （默认降级：跳过精排）
  └─ ⑥ LLM API 可用? ──是──→ 流式生成回答
            │否
...
**关键规则**：
- Milvus 不可用时，跳过整个检索链路，直接 LLM 回答（无引用来源）
- 任何降级路径必须在 metadata 中标注 `cache_hit: false` 和降级组件名
- 6 个依赖全部不可用时，返回 503："所有检索和生成服务暂时不可用"
...
### TD3: 测试覆盖深度
**决策**：两层覆盖：
- 单元测试：`header_recursive.py` 切分算法（纯函数），覆盖 H1/H2 边界、表格保护、跨页桥接、合并逻辑
- 集成测试：Milvus 上传 + 检索端到端（Mock Embedding API）
不需要 E2E 测试。

**Mock 策略**：
- Embedding API：用 VCR 录制真实响应回放，或固定 1024 维零向量（仅验证流程通断）
- Milvus：集成测试用 Docker 启动真实 Milvus（`docker compose up -d`），测试后 `down` 清理
- DashScope LLM/Reranker：Mock 返回固定 JSON（{"choices": [{"message": {"content": "测试回答"}}]}）
- Redis：Mock `get_cache_manager()` 返回 None，验证降级路径
