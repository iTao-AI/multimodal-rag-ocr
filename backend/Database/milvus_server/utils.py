...
from chatchat.settings import XF_MODELS_TYPES, Settings
...
def detect_xf_models(xf_url: str) -> Dict[str, List[str]]:
    ...
    """
    xf_model_type_maps = {
        "llm_models": lambda xf_models: [
            k
            for k, v in xf_models.items()
            if "LLM" == v["model_type"] and "vision" not in v["model_ability"]
        ],
        "embed_models": lambda xf_models: [
            k for k, v in xf_models.items() if "embedding" == v["model_type"]
        ],
        "text2image_models": lambda xf_models: [
            k for k, v in xf_models.items() if "image" == v["model_type"]
        ],
        "image2image_models": lambda xf_models: [
            k for k, v in xf_models.items() if "image" == v["model_type"]
        ],
        "image2text_models": lambda xf_models: [
            k
            for k, v in xf_models.items()
            if "LLM" == v["model_type"] and "vision" in v["model_ability"]
        ],
        "rerank_models": lambda xf_models: [
            k for k, v in xf_models.items() if "rerank" == v["model_type"]
        ],
        ...
        "text2speech_models": lambda xf_models: [
            k
            for k, v in xf_models.items()
    ...
    try:
        ...
        logger.warning(
            "auto_detect_model needs xinference-client installed. "
            'Please try "pip install xinference-client". '
        )
    except requests.exceptions.ConnectionError:
        logger.warning(
            f"cannot connect to xinference host: {xf_url}, please check your configuration."
        )
    except Exception as e:
...
def get_config_models(
    ...
    for m in list(get_config_platforms().values()):
        ...
        if m.get("auto_detect_model"):
            if (
                not m.get("platform_type") == "xinference"
            ):  # TODO：当前仅支持 xf 自动检测模型
                logger.warning(
                    f"auto_detect_model not supported for {m.get('platform_type')} yet"
                )
                continue
        ...
        for m_type in model_types:
            ...
            if models == "auto":
                logger.warning(
                    "you should not set `auto` without auto_detect_model=True"
                )
                continue
            elif not models:
                continue
...
def get_default_embedding():
    ...
    if Settings.model_settings.DEFAULT_EMBEDDING_MODEL in available_embeddings:
        ...
        logger.warning(
            f"default embedding model {Settings.model_settings.DEFAULT_EMBEDDING_MODEL} is not found in "
...
def get_default_graph() -> str:
    ...
    if Settings.tool_settings.DEFAULT_GRAPH in available_graphs:
        ...
        logger.warning(
            f"default graph {Settings.model_settings.DEFAULT_LLM_MODEL} is not found in available graphs, "
...
def get_history_len() -> int:
    return (
        Settings.model_settings.HISTORY_LEN
        or Settings.model_settings.LLM_MODEL_CONFIG["action_model"]["history_len"]
    )
...
def get_graph(
    name: str, llm: ChatOpenAI, tools: list[BaseTool], history_len: int
) -> Dict[str, Any]:
    """
    获取已注册的图
    :param name: 图的名称
    ...
    if name in graphs_registry._GRAPHS_REGISTRY:
        ...
        graph_instance = graph_info["func"](
            llm=llm, tools=tools, history_len=history_len
        )
...
def log_error_trace(e: Exception, operation: str, **context_kwargs: Any) -> str:
    """
    简化版的错误日志记录函数
    ...
    Args:
        e: 异常对象
        operation: 操作描述
        **context_kwargs: 上下文信息的关键字参数

    Returns:
        str: 格式化后的错误信息
    """
    return format_error_with_trace(
        e, operation, context=context_kwargs if context_kwargs else None
    )
