...
功能:
- 查询结果缓存 (避免重复检索)
- Embedding 结果缓存 (避免重复计算相同文本的向量)
- Redis 不可用时自动降级，不影响功能

部署: docker run -d --name redis -p 6379:6379 redis:alpine
"""
import json
import hashlib
import os
...
try:
    ...
    logger.warning("Redis 未安装，缓存功能将不可用。安装: pip install redis")
...
class CacheManager:
    ...
    def get_query_result(self, collection_name: str, query: str) -> Optional[List[Dict]]:
        """获取查询结果缓存"""
        key = self._generate_key("query_result", collection_name, query)
        return self.get(key)

    def set_query_result(
        self, collection_name: str, query: str, results: List[Dict], ttl: Optional[int] = None
    ) -> bool:
        """设置查询结果缓存"""
        key = self._generate_key("query_result", collection_name, query)
