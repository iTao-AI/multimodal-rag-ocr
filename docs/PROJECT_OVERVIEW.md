...
- 后端已有 pytest 用例，部分历史脚本仍需手动启动依赖服务
- 部分历史测试脚本包含本地路径假设，换机器前需要调整
- 测试用例使用 fake key / 环境变量占位，不应写入真实 API key
...
| API Key 管理    | 需持续检查 | 当前 tracked 文件仅保留 `.env.example`；真实 key 必须通过本地 `.env` 或环境变量注入 |
...
| 4   | 建立 API key 扫描和提交前检查                              | 1h    | 安全    |
| 5   | 无鉴权中间件                                           | 2-3d  | 安全    |
| 6   | 无 rate limiting                                  | 1d    | 成本控制  |
| 7   | 补齐前端 lint 配置和端到端测试覆盖                            | 1-2d  | 代码质量  |
| 8   | 无统一 docker-compose                               | 1-2d  | 部署体验  |
| 9   | 保持 README 与代码实现同步                                | 0.5d  | 可维护性  |
